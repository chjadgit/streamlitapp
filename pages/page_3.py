...
def select_bhdf(data, start_time, end_time, LOANSTATUS, index_select):
    ...
    if len(index_select) == 1:
        ...
        fig2 = go.Scatter(x=bf_sysdata[index_select[0]],
                          ...
                          name='C-M1')
        datas = [fig1, fig2]
        layout = go.Layout(title="{} 维度C-M1图".format(index_select[0]),
                           xaxis=dict(title=index_select[0]),
                           yaxis=dict(title="贷款余额"),
                           yaxis2=dict(title="C-M1", overlaying="y", side="right"),
                           )
...
if multipage == '时间维度':
    ...
    fig.update_layout(width=800,
                      ...
                      title_text="产品 {} C-M1".format(prodt_l5_up),
...
if multipage == '本行分析':
    ...
    st.markdown('#### 报告区间 {} 至 {} 各支行C-M1分析'.format(start_time, end_time))
